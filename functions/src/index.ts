...
function parseTimestampToSeconds(val: any): number | null {
  ...
  if (typeof val === 'string') {
    ...
    if (cleaned.includes(':')) {
      ...
      if (parts.length === 2) {
        ...
        if (!isNaN(mins) && !isNaN(secs)) return Math.max(0, Math.floor(mins * 60 + secs));
      } else if (parts.length === 3) {
        const hrs = parseFloat(parts[0]);
        const mins = parseFloat(parts[1]);
        const secs = parseFloat(parts[2]);
        if (!isNaN(hrs) && !isNaN(mins) && !isNaN(secs)) return Math.max(0, Math.floor(hrs * 3600 + mins * 60 + secs));
      }
    }
